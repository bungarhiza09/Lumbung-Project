...
export default function FoodRescue() {
  ...
  async function fetchDonasi() {
    ...
    // Ambil data donasi
    const { data: donasiData, error } = await supabase
      .from('donasi')
      .select(`
        *,
        donor:profiles!donasi_donor_id_fkey(
          id,
          nama,
          kota,
          kabupaten
    ...
    // Gabungkan warung ke donasi
    const merged = (donasiData || []).map(item => ({
      ...item,
      warung:
        warungData?.find(w => w.user_id === item.donor_id) || null
    }))
  ...
  const filtered =
    filter === 'semua'
      ...
      : donasi.filter(d => {
          ...
          return (
            d.kabupaten?.toLowerCase() ===
              profile.kabupaten?.toLowerCase() ||
            d.provinsi?.toLowerCase() ===
              profile.provinsi?.toLowerCase()
          )
        })
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <p className="text-xs text-white/70 leading-relaxed max-w-xs">
                  Hubungkan surplus makanan dari warung &
                  rumah tangga ke keluarga yang membutuhkan.
                </p>
              </div>
            </div>
            ...
            return (
              ...
              >
                ...
                <div className="p-4">
                  ...
                  {item.deskripsi && (
                    <p className="text-xs text-[#7a8a7a] mb-3 line-clamp-2">
                      {item.deskripsi}
                    </p>
                  )}
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    ...
                    {item.warung?.nama_warung && (
                      <span className="inline-flex items-center gap-1 text-xs text-[#d4720a] bg-[#fef3e7] border border-[#f9d4a7] px-2.5 py-1 rounded-full font-medium">

                        🏪 {item.warung.nama_warung}

                        {item.warung.is_verified && (
                          <span>✅</span>
                        )}

                        {item.warung.rating_avg > 0 && (
                          <span className="text-yellow-500">
                            ⭐ {item.warung.rating_avg.toFixed(1)}
                          </span>
                        )}
                      </span>
                    )}
